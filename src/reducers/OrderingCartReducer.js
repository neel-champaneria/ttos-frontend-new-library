import {
    ADD_TO_CART, CHECK_OUT, CHECK_OUT_FAIL, CHECK_OUT_SUCCESS,
    DECREASE_QUANTITY_FROM_CART, INCREASE_QUANTITY_FROM_CART, REMOVE_FROM_CART,
    CHECK_OUT_ORDER_STATUS, CHECK_OUT_ORDER_STATUS_TIMEOUT, CLEAN_CART, UPDATE_ORDER_STATUS_TIMEOUT,
    SAVE_ORDERING_INFO_TO_CART, CLEAN_ALL_CART, DONOT_APPLY_PROMOTION, APPLY_PROMOTION,
    ADD_BUY_ONE_GET_ONE_FREE_PROMOTION, REMOVE_BUY_ONE_GET_ONE_FREE_PROMOTION, RE_CALCULATE_ORDER_PRICE, CLEAN_REMOVED_PROMOTIONS_ARRAY, CLEAR_CHECKOUT_RESPONSE
} from '../constants';
...
export default (state = initialState, action) => {
    ...
    const calculateSequence = () => {
        ...
        currentOrderingItems.map(item => {
            if (maxlength < item.sequence)
                maxlength = item.sequence;
        })
        return maxlength;
    }
    ...
    switch (action.type) {
        case ADD_TO_CART:
            ...
            var orderPrice = Money.orderPrice(state, currentOrderingItems, state.removedPromotions, tempBuyOneGetOneFreePromotions);
            return {
                ...state,
                ...orderPrice,
        ...
        case INCREASE_QUANTITY_FROM_CART:
            currentOrderingItems.filter(item => {
                if (item.itemId === action.payload.itemId && item.sequence === action.payload.sequence) {
                    item.qty = item.qty + 1;
                    if ((item?.getFreeItemPromoId || 0) > 0) {
                        delete item.getFreeItemPromoId;
                    }
                    item.priceSum = Money.calcPrice(item.price, item.qty, item.modifierPriceSum);
                }
            });
        ...
        case SAVE_ORDERING_INFO_TO_CART:
            if (action.payload.deliveryFee === undefined || action.payload.deliveryFee === null) {
            ...
            }, currentOrderingItems, state.removedPromotions, tempBuyOneGetOneFreePromotions);
            return {
                ...state,
                ...action.payload,
                ...orderPrice,
        ...
        case DONOT_APPLY_PROMOTION:
            ...
            var orderPrice = Money.orderPrice(state, currentOrderingItems, tempRemovedPromotions, tempBuyOneGetOneFreePromotions);
            return {
                ...state,
                ...orderPrice,
        ...
        case ADD_BUY_ONE_GET_ONE_FREE_PROMOTION:
            ...
            if (!tempBuyOneGetOneFreePromotion.seq) {
                ...
                tempBuyOneGetOneFreePromotion.seq = `${tempBuyOneGetOneFreePromotion.id}_${state.buyOneGetOneFreePromotions.length + 1}`;
                tempBuyOneGetOneFreePromotions = [...tempBuyOneGetOneFreePromotions, tempBuyOneGetOneFreePromotion];
            } else {
                // Continue shopping Bogof promotion
                tempBuyOneGetOneFreePromotions = tempBuyOneGetOneFreePromotions.map(promo => {
        ...
        case REMOVE_BUY_ONE_GET_ONE_FREE_PROMOTION:
            tempBuyOneGetOneFreePromotions = tempBuyOneGetOneFreePromotions.filter(itm => itm.seq !== action.payload.seq);
