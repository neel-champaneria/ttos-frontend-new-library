import {format, getDay, getHours, getMinutes, isAfter, isBefore, isEqual, parse, addMinutes, subMinutes} from 'date-fns';
import {timeSlots} from '../constants';

export const singaporePhoneRegex = '^[6|8|9]\\d{7}$|^\\+65[6|8|9]\\d{7}$|^\\+65\\s[6|8|9]\\d{7}$';
...
export const checkShowChangeTimeBtn = (resOrders, minPrepareTime) => {
    ...
    const tempOrders = resOrders?.map(order => {
        const orderDeliveryDateTime = order?.deliveryDateTime ? new Date(order.deliveryDateTime) : null;
        if (orderDeliveryDateTime != null) {
            if (orderDeliveryDateTime.getTime() >= new Date().getTime() + parseInt(minPrepareTime, 10) * 60 * 1000 + 30000) {
...
export const getNewTimeWithAdvance = (time, advance) => {
    ...
    var openTimeInMins = parseInt(openTimeParts[0], 10) * 60 + parseInt(openTimeParts[1]) + advance;
    var newTime = zeroPad(parseInt(openTimeInMins/60, 10), 2) + ":" + zeroPad(parseInt(openTimeInMins%60, 10), 2);
    return newTime;
};

export const verifyCheckoutDateTime = (isPreOrder, deliveryDateTime, openingDaysArr, openTime, closeTime, advance) => {
    var checkoutTimeOk =  true;
    if (openTime > closeTime) {
        console.log("Closing next day");
        var checkoutDateTime = new Date();
        if (isPreOrder) {
            checkoutDateTime = parse(deliveryDateTime, "dd-MM-yyyy HH:mm:ss", new Date());
        }
        if (isBefore(checkoutDateTime, new Date())) {
            ...
            if (checkoutTime < openTime) {
                ...
                checkoutHours += 24;
                checkoutTime = checkoutHours + ":" + checkoutTimeParts[1] + ":" + checkoutTimeParts[2];
            }
            ...
            checkoutTimeOk = checkoutTime >= openTime
                && checkoutTime <= newCloseTime
                && (openingDaysArr.includes("" + checkoutDay) || (!openingDaysArr.includes("" + checkoutDay) && checkoutTime < newCloseTime));
        }
        ...
        checkoutTimeOk = checkoutTime >= openTime
            ...
            && (openingDaysArr.includes("" + checkoutDay) || (!openingDaysArr.includes("" + checkoutDay) && checkoutTime < closeTime));
        console.log(checkoutTimeOk);
    }
...
export const generateOpenCloseTimeOfDay = (deliveryDate, orderingMethod, openingDaysObj, specialDaysObj, pauseServiceObj, advanceObj) => {
    ...
    for (let i = 0; i < openingDatas.length; i++) {
        ...
        const openingDaysArr = openingData['opening_days']?.split(',').filter(Boolean)?.sort((a, b) => a - b);
        ...
        openingDaysArr.map(d => {
            if ((d - 1) == deliveryDay) {
                if (closeTime > openTime) {
                    ...
                    console.log('openCloseTimeOfDaysObj===>',openCloseTimeOfDaysObj, deliveryDay);
                    if (openCloseTimeOfDaysObj?.hasOwnProperty(deliveryDay)) {
                        let temp =  openCloseTimeOfDaysObj[deliveryDay];
                        temp.push({
                                timeFrom: "00:00",
    ...
    if (specialDaysObj.length > 0) {
        for (let i = 0; i < specialDaysObj.length; i++) {
            ...
            const specialOperationDateFromObj = parse(specialOperationDateFrom, 'dd-MM-yyyy', new Date());
            const specialOperationDateToObj = parse(specialOperationDateTo, 'dd-MM-yyyy', new Date());

            // Check if delivery date is in special day
            let inSpecialDay =
                (isAfter(deliveryDateObj, specialOperationDateFromObj) || isEqual(deliveryDateObj, specialOperationDateFromObj))
                && (isBefore(deliveryDateObj, specialOperationDateToObj) || isEqual(deliveryDateObj, specialOperationDateToObj));
            // console.log('inSpecialDay =====>', inSpecialDay);

            const specialOperationServices = specialDays.specialOperationServices.find(itm => itm.service === orderingMethod) || specialDays.specialOperationServices.find(itm => itm.service === 'opening');
            ...
            if (specialOperationServices?.id > 0) {
                ...
                if (advance > 0) {
                    ...
                    advanceBetween.map((itm, idx) => {
                        const openTimeAdvance = getNewTimeWithAdvance(itm.timeFrom, advance);
                        ...
                        if (closeTimeAdvance < openTimeAdvance) {
                            closeTimeAdvance = (parseInt(closeTimeAdvance.substr(0, 2), 10) + 24) + closeTimeAdvance.substr(2) + "";
                        }
                        if (closeTimeAdvance >= "23:59")
                            closeTimeAdvance = "24:00";
    ...
    if (pauseServiceObj && pauseServiceObj?.pauseService?.includes(orderingMethod)) {
        // console.log('openCloseTimeArr before pause ===>', openCloseTimeOfDaysObj[deliveryDay]);
        let newOpenCloseTimeArr = deepClone(openCloseTimeOfDaysObj[deliveryDay], []);
            ...
            if (restOfDay) {
                ...
                for (let i = 0; i < newOpenCloseTimeArr.length; i++) {
                    ...
                    if (startPTime <= timeFrom && endPTime >= timeTo) {
                        continue;
                    } else if (startPTime <= timeFrom && endPTime < timeTo && endPTime > timeFrom) {
                        newRange.timeFrom = endPTime;
                        newRange.timeTo = timeTo
                        tmpOpenCloseTimeArr.push(newRange);
                    } else if (startPTime > timeFrom && startPTime < timeTo && endPTime >= timeTo) {
    ...
    if (!isDeliveryDateAfterToday) {
        ...
        newOpenCloseTimeArr = newOpenCloseTimeArr?.filter(obj => {
            return ((curTimeStr < obj.timeTo && curTimeStr > obj.timeFrom) || (curTimeStr < obj.timeFrom));
        });
        openCloseTimeOfDaysObj[deliveryDay] = newOpenCloseTimeArr;
...
export const generateTimeArr = (openTimeOfDeliveryDayStr, closeTimeOfDeliveryDayStr, deliveryDate) => {
    const _timeArr = [];
    const isDeliveryDateAfterToday = isAfter(parse(deliveryDate, 'dd-MM-yyyy', new Date()), new Date());
    const curTimeStr = format(new Date(), 'HH:mm');
    let maxHour = 24;
    ...
    let minHour = curHour;
    if (curTimeStr < openTimeOfDeliveryDayStr.substr(0, 5) || isDeliveryDateAfterToday) {
    ...
    for (let i = 0; i <= remainingHour; i++) {
        ...
        minIntervalArr.forEach(minInterval => {
            if ((curMinute <= parseInt(minInterval, 10)) || i > 0 ||  minHour > curHour) {
                const _time = `${hourStr}:${minInterval}:00`;
                if (minHour > curHour) {
                    if (`${hourStr}:${minInterval}` >= openTimeOfDeliveryDayStr && `${hourStr}:${minInterval}` <= closeTimeOfDeliveryDayStr)
                        _timeArr.push(_time);
                } else {
                    if (`${hourStr}:${minInterval}` >= openTimeOfDeliveryDayStr && `${hourStr}:${minInterval}` <= closeTimeOfDeliveryDayStr)
                        _timeArr.push(_time);
    ...
    if ((maxHourMin % 15) == 0) {
        if (minHour < maxHour && _timeArr[_timeArr.length - 1] < closeTimeOfDeliveryDayStr) {
...
export const generateTimeArrWithSlot = (openTimeOfDeliveryDayStr, closeTimeOfDeliveryDayStr, deliveryDate, slot) => {
    const _timeArr = [];
    const isDeliveryDateAfterToday = isAfter(parse(deliveryDate, 'dd-MM-yyyy', new Date()), new Date());
    const curTimeStr = format(new Date(), 'HH:mm');
    let maxHour = 24;
            ...
            if (!found) {
                if ((curMinute <= parseInt(minInterval, 10)) || i > 0 || minHour >= curHour) {
                    ...
                    if (!found) {
                        ...
                        if (minHour >= curHour) {
                            if (_time >= openTimeOfDeliveryDayStr && _time <= closeTimeOfDeliveryDayStr) {
                                startTime = _time;
                                found = true;
                            }
                        } else {
    ...
    if (found) {
        ...
        } while (startTime <= closeTimeOfDeliveryDayStr + ":00" && temp < startTime);
    }
    return _timeArr;
};
